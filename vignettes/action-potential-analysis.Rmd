...
> It is important that cursor 2 touches the curve at a value that is lower than the voltage value for cursor 1. If cursor 1 is lower, then Clampfit may identify the location at cursor 1 as the after-hyperpolarization region if you forget to move the cursor to the threshold (see step 11).
...
```{r cursor-image-revised, fig.align = 'center', fig.alt = "A screenshot of Clampfit showing cursors positioned around the first action potential. The first cursor is now located at the x-value where the membrane potential is equal to the threshold.", fig.cap = "Note how Cursor 1 is shifted to the right. It is located a the x-value where the membrane potential crosses the threshold." echo=F}
...
16. Repeat this process for the current clamp step protocol taken after a hormone or treatment.

> If there are no action potentials in the later recording, insert the values for the columns `Letter` and `State` and leave all the others blank.
...
16. Press the `Enter` key and use `,` and `.` (or `<` and `>`) to cycle through the sweeps. Count the number of action potentials in each sweep.
