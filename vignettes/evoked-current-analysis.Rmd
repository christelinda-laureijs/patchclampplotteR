...
```{r evoked-currents-schematic, fig.align = 'center', fig.alt = "A sketch showing a horizontal line with two downward-pointing peaks separated by a short distance. These are the two evoked currents.", fig.cap = "This drawing shows the first and second evoked currents. The ratio between them is the paired-pulse ratio (PPR)", echo=F}
...
```{r evoked-currents-statistics, fig.align = 'center', fig.alt = "A screenshot of the correct statistics options selected in Clampfit.", echo=F}
...
## FAQ about evoked current analysis
...
*What do I do if there are two peaks?*
...
However, when writing a paper or thesis, you will use just one master csv file which contains the evoked current data for all recordings in the entire project. To append new data to this master csv, use the `add_new_cells()` function. Please see the Getting Started vignette for more details.
