---
title: "Tips for Code Organization"
description: >
  Learn how to write clean and organized code, avoid lengthy documents, and display key plots efficiently.
editor_options: 
  markdown: 
    wrap: 72
---

```{r set-up-document, include = FALSE}
...
you can use `source()` to refer to external `.R` scripts. This will make
your documents even shorter and enable you to split up into different
files for analyzing data, creating plots, and later, even writing
different chapters of a thesis. You can even export your plots and into
an attractive, easy-to-read HTML document to make it easy to show to
others.
...
```{r sample-outline, echo=T, eval=F}
# Data analysis
...
```{r outline-tool, out.width="25%", fig.alt = "A screenshot of the Outline Tool available in the upper-right corner of the Source pane. It features the headers that were listed with hashtags in the document."}
knitr::include_graphics("figures/Outline-tool.png")
```
...
```{r outline-tool-bottom-left, out.width="25%", fig.alt = "A screenshot of the Outline Tool available in the bottom-left corner of the Source pane. It features the headers that were listed with hashtags in the document and chunk names."}
knitr::include_graphics("figures/Outline-tool-bottom-left.png")
```
...
```{r good-code-example, echo=T, eval=F}
penguins %>%
...
- Could you use functions like `across()` from `dplyr` instead of
  repeating the same function for each column?
...
```{r sample-options-set, echo=T, eval=F}
knitr::opts_chunk$set(
  dev = "png",
  fig.align = "center",
  out.extra = "",
...
```{r sample-thesis-global-options, echo=T, eval=F}
knitr::opts_chunk$set(
  dev = c("cairo_pdf"),
  dpi = 600,
  fig.width = 12,
...
```{r source-scripts, echo=T, eval=FALSE}
...
Install the package and then load it with `library(patchwork)`. You can
...
in a 2 \times 2 pattern. I encourage you to read the [patchwork package website](https://patchwork.data-imaginist.com) for more useful tips!


```{r make-sample-plots}
raw_plot <- plot_raw_current_data(
  data = sample_raw_eEPSC_df %>% filter(letter == "AO"),
...
summary_plot <- plot_summary_current_data(
...
ppr_plot <- plot_PPR_data_single_treatment(
  data = sample_PPR_df,
  plot_treatment = "Control",
  plot_category = 2,
...
Here is an example. I used `fig.width = 12, fig.height = 10` in the chunk options.

```{r basic-patchwork-theme, echo = T, eval=F, fig.width = 12, fig.height = 10}
(raw_plot + summary_plot)/
  (ppr_plot + ap_trace)
```


```{r patchwork-example-1, fig.alt = "A screenshot of a figure with four subfigures arranged in a 2x2 grid."}
knitr::include_graphics("figures/patchwork-example-1.png")
```

To add figure captions, just add the `plot_annotation()` function. Previously I also defined a `ggplot` theme that makes the figure captions **bold**:

```{r make-patchwork-theme, echo=T}
...
Use the `+` sign to add the `plot_annotation` and use the `&` symbol to add a ggplot theme (your own or a pre-built theme). 

```{r sample-with-figure-numbers, echo = T, eval = FALSE, fig.width = 12, fig.height = 10}
patchwork_example_2 <- (raw_plot + summary_plot)/
  (ppr_plot + ap_trace) + plot_annotation(tag_levels = "A") & patchwork_theme
```

```{r patchwork-example-2, fig.alt = "A screenshot of a figure with four subfigures arranged in a 2x2 grid. This time, each subfigure has a capital letter label."}
knitr::include_graphics("figures/patchwork-example-2.png")
```

To add a figure caption, use the `fig.cap = "Insert figure caption here"` chunk option. 

> Pro tip! If your figure caption gets too long, you can separate out knitr chunk options onto separate lines using `#|`. I will show an example below:
...
If your figure captions get too long, I highly recommend naming them as separate objects with names like `chapter_2_fig_1_cap` so you can easily refer to them.
...
```{r sample-plot-viewer-code, out.width="50%", fig.alt = "A screenshot of the rmarkdown document that I use. Level one heading is glutamate. Level 2 heading is Insulin with a tabset command. Level 3 headings are Male and Female. Within these level 3 subheadings there are chunks using the make facet plot code to produce raw plots."}
knitr::include_graphics("figures/Plot-viewer-demo.png")
```
