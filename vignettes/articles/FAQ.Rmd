...
Let's say that you are running a plotting function repeatedly, but only changing one argument at once (like `treatment`). To reduce repetitive code, you can use `pmap()` from the `purrr` package to run a function with a list of treatments.
...
Then, I'm using `purrr::pmap()` using the `sample_treatment_names_and_colours()` dataset. I will use the `treatment` column in the `treatment` argument.

```{r eval=T, include=T}
pmap(.l = list_of_treatments,
     .f = ~ with(
       list(...),
