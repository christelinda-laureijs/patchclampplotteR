...
Before you begin, you should set up a .csv file with the following columns. Note that most are in lowercase. 
...
> *Warning!* It is important that Cursor 2 touches the curve at a value that is lower than
...
14. Select all data except for the `File Path` column (unless you think you will
...
16. While you still have this file open, you may want to count action potentials (see the section on [counting action potentials](#count) below).

17. A completed dataset should look something like the spreadsheet below.

> *Note* If there are no action potentials in the later recording, insert the values
...
18. Create a separate .csv file with four columns:
...
18d. The entire range from `C2` to `C11` should be selected. Move your mouse to the bottom right corner of the cell until you see the `+` symbol again. Hold down the `Ctrl` key and drag this selection down. This will generate the sequence from 1-10 repeatedly.

18e. You can then copy and paste this range repeatedly to fill your column.

19. Open a current clamp steps recording in Clampfit (you may be coming directly from step 16 when you [measured action potential properties](#measure).)

20. Press the `Enter` key to view one sweep at a time, and use `<` and `>` to cycle through
the sweeps. Count the number of action potentials in each sweep, and record them in the `no_of_APs` column.
