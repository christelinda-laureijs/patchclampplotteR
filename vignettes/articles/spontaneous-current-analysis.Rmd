...
* Next, click on (`Window` -> `Results1` or `Alt+w+2`) and then `File` -> `Save As` (`Ctrl+s`). I would suggest creating a consistent naming convention, such as `Letter`-events, as in `AA-events.atf`, `AB-events.atf`, etc.
...
> Note! If your baseline is longer than 5 minutes, you should ensure that you take the last 5 minutes of this recording. Otherwise, you may be misrepresenting the sections of the recording that belong in the baseline region!
