...
In this method, the threshold is five times the standard deviation of the noise amplitudes during a flat baseline region. This is comparable to the root mean square noise (RMS) used in MiniAnalysis.
...
9. On the results page, copy the values in the `R1S1 Peak` column. Paste these into the A column of a blank Excel sheet.
...
> If you need to remove an entire trace, click on it and press the `Delete` key. This will hide the trace Later, you will select visible traces for the analysis.
...
```
> If you do not check "Full Scale X axis", Clampfit will continuously scroll through the recording and it will take much longer to run.
...
> If you do not complete this step, Clampfit will try to refresh and resize this window each time a current is detected. It will likely freeze and crash the software.
...
22. When the Event Detection has stopped, the nonstop button will become greyed out there will be a small notification sound. This is what the recording will look like after.


```{r finished, out.width="25%", fig.alt = "A screenshot of the recording after Event Detection is complete. Currents are highlighted in blue."}
knitr::include_graphics("figures/Event-detection-finished.png")
```

## Exclude evoked currents

23. Since this analysis focuses on spontaneous currents only, you will need to exclude any stimulus artifacts and evoked currents that were included in the event detection. Clampfit does not have an easy way to select multiple currents at once. However, one work-around is to create a plot of current amplitude over time and select points to delete.

24. Click on the `Define Graphs` button 

25


_##. Click on the Results sheet (`Window` -> `Results1`) and `Save As`. I would suggest creating a consistent naming convention, such as `Letter`-events, as in `AA-events.sta`, `AB-events.sta`, etc.
