...
knitr::include_graphics("figures/noise-vs-currents-diagram.png")
```
...
12. In cell B3 type `=B2*5`. This value is the threshold value (write it down in your lab book).

13. You may want to keep this Excel sheet for later use (just replace the data in the A column each time).
...
18. Set the search region to `Full Trace`. Click the `Select` button and ensure that all visible traces are selected.

```{r search-box, fig.alt = "A screenshot of the Event Detection Search Box in Clampfit."}
...
knitr::include_graphics("figures/search-parameters-button.png")
```
...
> Be sure to uncheck "Automatically open Event Viewer window"! If you do not do this, Clampfit will continuously refresh this window each time a new event is detected, leading to crashing and lagging software.

21. Click on the Nonstop button
