...
> **WARNING!!** If you are starting a brand new project, the `.csv` in the `old_raw_data_csv` argument MUST contain only the `letter` column name. This would typically go in cell A1 if you are using Excel. If you try to use a completely empty `.csv` sheet, R will not recognize it as a valid `.csv`. 
...
Hopefully this vignette has given you an idea of some of the plotting functions that this package can do. The documentation for each function contains lots of additional information about each argument, and you can also explore the articles for [Evoked Current Analysis](https://christelinda-laureijs.github.io/patchclampplotteR/articles/evoked-current-analysis.html) and [Action Potential Analysis](https://christelinda-laureijs.github.io/patchclampplotteR/articles/action-potential-analysis.html).
...
```{r ruby-picture, echo=F, fig.alt = "A picture of a knitted white rat sitting on a keyboard and working at a computer.", fig.cap = "Ruby likes to help us with data analysis. She says 'Have fun!'"}
