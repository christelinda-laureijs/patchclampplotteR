...
Click on `File` -> `New File` -> `R Markdown`. Save this somewhere (I like to store this in the `Thesis` subfolder). You're now ready to start your analysis!
...
You may also find the following packages helpful. The `here` package will help R find your files in the correct subfolders in your project folder. `dplyr` will allow you to rearrange, sort, filter, and change your data, and `ggplot2` will allow you to make custom plots with your data.

Place the `library()` code somewhere near the top of your .Rmd file so everything is in one place, and the functions are loaded for future analyses.
...
First, you must create a `.csv` file containing information about factors such as
...
```{r treatment-schematic, echo=F, out.width="100%", out.extra = "", fig.alt = "A diagram showing two rows of data. The first has the text Category = 2 (add insulin), while the second row has Treatment, followed by the words Control, HNMPA, PPP and HNMPA and PPP combined. The goal of this figure is to show that treatments are nested under category."}
...
Next, I will double-check the raw evoked current data that has been copied over from
...
> Notice: All of these files are stored in the `Data/`subfolder, which is why the filenames start with "Data/".
...
So far, this complete dataset is only stored as an object (`first_time_df`) in R. You should save this as a `.csv` on your computer so you can access it later. Once you have confirmed that your files were merged without any issues, change `write_new_csv = "no"` to `"yes"`, and write a filename in `new_file_name.`

> **Warning!!** You MUST specify a filename in `new_file_name`, and it must end with ".csv".
...
```
If the process is successful, you will see a confirmation message displaying the name of the saved file.
...
By making the `new_file_name` equal to `old_raw_data_csv`, this effectively overwrites your data each time, adding your new data as rows to the bottom of your existing datasheet.
...
You can comment out this code for now, as you don't need to repeatedly run it each time you run your RMarkdown file. Just un-comment it and run it whenever you have new data.

`add_new_cells()` also has the ability to import spontaneous data, action potential parameter data, and action potential count data. Please see the documentation for `add_new_cells()` for more information!
...
You can use `dplyr` functions to quickly explore your data. Here's how to do three common analyses:
...
```{r presentation-slice-counts, eval=F, echo=T}
...
```{r actual-cell-counts-results, eval=T, echo=F}
cell_counts <- raw_eEPSC_df %>%
  select(-animal) %>%
  tidyr::separate_wider_delim(cell,
    delim = ".",
...
reactable(cell_counts)
```

#### Identify maximum recording length

This is particularly helpful to see if there are recordings that don't meet the length requirements for a t-test (you may get errors if some recordings are shorter than others).


```{r get-max-times-display, eval=F, echo=T}
raw_eEPSC_df %>%
  group_by(letter) %>%
  summarize(max_time = max(time))
```

```{r actual-max-times, echo=F, eval=T}
raw_eEPSC_df %>%
  group_by(letter) %>%
  summarize(max_time = max(time)) %>%
  reactable()
...
After you have finished a brief exploration of your data, it is time to analyze it! You must read in the FULL .csv file that you created and saved with `add_new_cells()`. This is the dataframe that has columns for `letter`, `ID`, `P1`, `P2`, `age`, `sex`, `days_alone`, etc.

> Do NOT use the raw dataframe that has only 4 columns!
> You MUST import the .csv file that you created with `add_new_cells(), where write_new_csv = "yes"!
...
> If you want to save your plots, set save_plot_png = "yes" in the function below!
...
`plot_raw_current_data()` will return a list of ggplot objects. To see all of your plots at once, type the name of the object that you just made (`raw_eEPSC_control_plots`):

```{r echo=T, eval=F}
raw_eEPSC_control_plots
```

If you want to observe just one specific plot, you can select it by letter.  
...
You may want to modify one of these plots and save it on its own as a representative cell for a poster or publication. One common task is to remove the title and subtitle.
...
You can then save this plot using `ggsave()` from `ggplot2`. Remember, if you want to save all of your plots, set `save_plot_png = "yes"` when running the `plot_raw_current_data()`. This `ggsave()` is just for if you want to save a plot that is heavily modified from the default.
...
It is often useful to summarize the data per minute. If you are familiar with GraphPad Prism's "prune rows" function, `make_pruned_EPSC_data()` will perform
the same function.
...
```{r plot-pruned-current-data, eval=T, echo=F}
...
```{r display-make-pruned-plots, eval=F, echo=T}
pruned_eEPSC_control_plots <- plot_raw_current_data(
...
> Quick tip! If you need help for a specific function, type `?` followed by the name of the function. For example, if you type `?plot_raw_current_data` into the Console and press Enter, this will open the help page, with information about how to use each argument in the function, and there are examples to get you started (just replace the example code with your data).

### Step 3: Summarize data

The next step is to group the data by treatment by sex and obtain summary data. `make_summary_EPSC_data()` will generate a list of 3 dataframes. One dataframe (accessible with `$summary_data`) grouped the data into intervals and generated summary statistics (like mean and standard error) for each point. The interval length was already
specified during the `make_normalized_EPSC_data()` function from earlier.

```{r make-summary-data, eval=T, echo=F}
...
```{r display-make-summary-data-frame, eval=F, echo=T}
summary_eEPSC_df <- make_summary_EPSC_data(
  data = raw_eEPSC_df,
  current_type = "eEPSC",
  save_output_as_RDS = "no",
...
The second dataframe of the summary data (accessible with `$percent_change_data`) contains information on the percent change in evoked current amplitude `percent_change` during a specific time interval (`ending_interval`) relative to the baseline (`baseline_interval`). For example, if currents decreased by 50% after the hormone, the value of `percent_change` is 0.5.
...
A third dataframe (accessed with `$mean_SE`) contains summary information that may be useful for publications. For example, you may want to report the mean ± SE before and after applying a hormone, along with *n*, the sample size.

```{r eval=F, echo=T}
summary_eEPSC_df$mean_SE
```

This dataset contains the values for mean, standard deviation, standard error, and sample size, *n*, both before and after protocol or hormone. The time of the "after" period comes from what you would have entered as the "ending_interval" when you ran `summary_eEPSC_df()` above.

```{r echo=F, eval=T}
reactable(summary_eEPSC_df$mean_SE)
```
...
> **NOTE!** You will be making summary plots with the **pruned data** (`$all_cells`) dataframe, but you will be running t-tests with the **summary data** (`$summary_data`)!
...
> Be sure to check the assumptions of your data! If the data do not pass assumptions of normality and homogeneity of variables, use `test_type = "pairwise.wilcox.test"`.
...
> Quick tip! Want to see how I built some of these functions? Type the name of the function without the () brackets into the console and then hit enter (for example, type `plot_summary_current_data`). You should then see the raw code used to produce this function. This is helpful if you want to use a geom similar to mine (like the sina plot) and want to see what code to use.

> Quick tip #2! If you need help, the [R Graphics Cookbook](https://r-graphics.org/) is a great place to start! If you need inspiration on what type of graph to make, look at the [R Graph Gallery](https://r-graph-gallery.com/)!

### Looking at cell characteristics

 You can use your summary data (particularly the `$percent_change_data`) to make some interesting plots. Using `cell_characteristics` data throughout makes it very easy to answer interesting questions about your data. For example, does the baseline evoked current amplitude (data from `t0to5`) affect a cell's insulin sensitivity? I can use `percent_change` as a marker of the effect size of insulin.
...
```{r plot-one-ppr-treatment-facet, fig.alt = "A scatterplot of paired-pulse ratio over time (baseline or post-insulin) showing no difference in the paired pulse ratio across these two times. The plot is divided by sex", warning=FALSE}
plot_PPR_data_single_treatment(
  data = PPR_df,
  plot_treatment = "Control",
  plot_category = 2,
...
The plotting functions provided in `patchclampplotteR` are actually just shortcuts to make plots with ggplot. All the data that you need is provided with functions like `make_summary_EPSC_data()`, `make_variance_data()` and `make_PPR_data()`. Why don't you try making your own plots in ggplot?

As an example, here is the same `$percent_change_data` from `make_summary_EPSC_data()` presented in **8** different ways!! I haven't included the code and data here because this is from my own research, but the list below should give you a good idea of what functions you could use to create similar plots.

 * **A)** *Grouped categorical scatterplot* with `geom_point(position = position_dodge)` to separate the sexes.
 ...
 * **G)** *Barcode plot * (this is just `geom_point(shape = 95, size = 10)`, with `alpha = sex`)
 * **H)** *Half-eye plot* using `stat_halfeye()` from the `ggdist()` package


```{r same-data-multiple-ways, echo=F, fig.align='center', fig.alt = "A series of plots showing percent change in evoked current amplitude after 25 minutes of adding insulin. There are eight plots that all show the same data but in different ways. For example, figure A shows a grouped scatterplot, figure B shows a sina plot, figures C and D show violin plots. Figures E and F show boxplots, and Figure G shows a strip plot. Figure H shows a half-eye plot."}
knitr::include_graphics("Same-data-8-ways.png")
```
...
And, please be sure to check out the [Reference page](https://christelinda-laureijs.github.io/patchclampplotteR/reference/index.html) and read the documentation for each function. Each function has many additional arguments that I didn't cover here, and there are some functions there that may help you to create the plot you need!
