...
cell_counts <- raw_eEPSC_df %>%
  ...
  summarize(no_animals = n_distinct(animal),
...
raw_eEPSC_df %>% 
  group_by(letter) %>% 
  summarize(max_time = max(time)) %>% reactable()
```
...
You may want to remove the title and subtitle for a poster or publication.
...
You can then save this plot using `ggsave()` from `ggplot2`.
