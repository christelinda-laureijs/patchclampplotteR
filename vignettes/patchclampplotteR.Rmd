...
```{r}
# import_ext_data("empty_eEPSC_sheet.csv")
```
...
```{r}
# read.csv(here::here("inst/extdata/empty_raw_eEPSC_datasheet.csv"))
```
...
Here is a quick example of what a blank data sheet looks like after you have added raw data for the first time:

```{r first-time-df}
# reactable(first_time_df)
```
...
# utils::write.csv(first_time_df, here::here("Insert-filename-here.csv"), row.names = F)
```

This is an example of what the full few rows look like now:

```{r}
# head(first_time_df, n = 5) %>% 
#   reactable()
```
...
You can use `dplyr` functions to quickly explore your data. Here are just some examples of what you can do.

### Count number of cells per sex and treatment

Run this line of code on the raw data. Here, I filtered the data to category 2 only (experiments where I added insulin) and grouped by treatment. I then counted the number of cells per sex.
